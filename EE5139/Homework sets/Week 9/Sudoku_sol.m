...
for i = 1:9
    J = perms([1:(i-1),(i+1):9]);
    for counter = 1:size(J,1)
        j = dot(v2n, (J(counter,:)-1))+1;
        T(i,j) = 1;
    end
end
clear('v2n','i','j','J','counter');
% Initialization 
for i = 1:9
    for j = 1:9
        if pre(i,j)>0
...
for a = 1:9
    for k = 1:9
        ...
        in = 1;
        for j = [1:(k-1),(k+1):9]
            in = kron(in, transpose(m{a,j, 1}));
        end
        out = T*in;
        n_row{a,k} = transpose(out./sum(out));
        clear('j','in','out');
    end
end
clear('a','k');
...
for b = 1:9
    for k = 1:9
        ...
        in = 1;
        for i = [1:(k-1),(k+1):9]
            in = kron(in, transpose(m{i,b, 2}));
        end
        out = T*in;
        n_col{b,k} = transpose(out./sum(out));
        clear('i','in','out');
    end
end
clear('b','k');
...
for c2 = 1:3
    ...
    for k2 = 1:3
        ...
        for i = 1:3
            for j = 1:3
                if i ~= k1 || j ~= k2
                    in = kron(in, transpose(m{3*(c1-1)+i,3*(c2-1)+j, 3}));
                end
            end
        end
        out = T*in;
        n_block{c1,c2,k1,k2} = transpose(out./sum(out));
        clear('i','j','in','out');
    end
    end
end
...
for j = 1:9
	if pre(i,j)==0
        ...
        m_new{i,j,1} = n_col{j,i}.*n_block{c1,c2,k1,k2};
        m_new{i,j,1} = m_new{i,j,1}./sum(m_new{i,j,1});
        m_new{i,j,2} = n_row{i,j}.*n_block{c1,c2,k1,k2};
        m_new{i,j,2} = m_new{i,j,2}./sum(m_new{i,j,2});
        m_new{i,j,3} = n_row{i,j}.*n_col{j,i};
        m_new{i,j,3} = m_new{i,j,3}./sum(m_new{i,j,3});
        clear('c1','c2','k1','k2');
	end
end
end
