...
// allocate memory to contain the whole file.
  ...
  /*** the whole file is loaded in the buffer. ***/
	...
	while(ci<= lsize)
	{
		...
		n = sendto(sockfd, &sends, strlen(sends), 0, addr, sock_len); 
		if(n == -1) {
		...
		int ack_fd;
		result= recvfrom(sockfd, &ack, 2, 0, addr, &addrlen);
		...
		else
		{
			...
			printf("ACK result: %d \n",result);
			printf("In this time, this ack socket(ack_fd):%d\n",sockfd);
		}
		...
		//close(sockfd);
		//close(result);
		printf("This is ok for while test\n");
	}
