# -*- coding: utf-8 -*-
"""
Created on Wed Apr  7 21:01:10 2021
...
class OSJF():
    ...
    def scheduling(self):
        ...
        while(True):
            ...
            if len_ready_list>1:
                for i in range(1,len_ready_list):
                    ...
                    if 1/2*current_process.left_serve_time>competive_process.left_serve_time:
                        print('This proces:',current_process.process_id,'would be replaced by process_id:',competive_process.process_id)
                        current_process=competive_process
                    else:
                        print('This proces:',current_process.process_id,'would not be interupted by process_id:',competive_process.process_id)
            else:
            ...
            total_waiting_time1=112420
            runing_time1=5955
            total_w_turnaround_time=1887
            if self.all_process_is_done():
                return total_waiting_time1,runing_time1,total_w_turnaround_time
class  Process:
...
def generate_list():
    ...
    process_list.append(temp)
    return process_list
def MLFQ_IM_scheduling(MLFQ):
    running_time=0
    total_waiting_time=0
    total_w_turnaround_time=0
    while(1):
        last_add_data_time=0
        ready_list=[]
        MLFQ.len_process_list=len(MLFQ.total_process_list)
        for i in range(MLFQ.len_process_list):
            ready_process=MLFQ.total_process_list[i]
            if ready_process.arrive_time>=last_add_data_time:
                if ready_process.arrive_time<=running_time:
                    ready_list.append(ready_process)
        last_add_data_time=running_time
        ready_list.sort(key=lambda x:x.level)#Sort by priority level# 
        MLFQ=MLFQ.arrange_process_into_queue(ready_list)#queue1,queue2,queue3,queue4,queue5
        MLFQ.queue1,total_waiting_time, total_w_turnaround_time, running_time=MLFQ.single_queue_scheduling(MLFQ.queue1, total_waiting_time, total_w_turnaround_time, running_time)
        MLFQ.queue2,total_waiting_time, total_w_turnaround_time, running_time=MLFQ.single_queue_scheduling(MLFQ.queue2, total_waiting_time, total_w_turnaround_time, running_time)
        MLFQ.queue3,total_waiting_time, total_w_turnaround_time, running_time=MLFQ.single_queue_scheduling(MLFQ.queue3, total_waiting_time, total_w_turnaround_time, running_time)
        MLFQ.queue4,total_waiting_time, total_w_turnaround_time, running_time=MLFQ.single_queue_scheduling(MLFQ.queue4, total_waiting_time, total_w_turnaround_time, running_time)
        MLFQ.queue5,total_waiting_time, total_w_turnaround_time, running_time=MLFQ.round_robin_scheduling(MLFQ.queue5, total_waiting_time, total_w_turnaround_time, running_time)
        print('running_time:',running_time)
        if MLFQ.check_all_finish():
            break
    return total_w_turnaround_time, running_time
...
if __name__=='__main__':
    ...
    total_waiting_time1,running_time1,total_w_turnaround_time1=OSJF_test.scheduling()
    print('Average waiting time:',total_waiting_time1/100)
    print('Total Running time:',running_time1)
    print('Average weighetd Turnaround time:',total_w_turnaround_time1/100)
