...
close all
%This is for Q4
%This code will run for longer time(about 10 minutes) compared with Q1 Q2 and Q3 ,because I use
...
%----------------train-------------------
[train_num ,num_feature_label] = size(data.ytrain); %number of train samples
[test_num ,num_feature_label] = size(data.ytest);%number of test samples
...
for kk = 1:length(K)  
    ...
    for i = 1:train_num
        ...
        class1 = sum(class(1:K(kk)));   %count number of class 1 in the nearest K samples ,  
        if class1 >=K(kk)/2       %if there are just half samples of class1 ,in the nearest K samples(like 3 in 6),we still predict sample to be class1
            class_pre(i) = 1;       
        end
...
plot(K,Error_tr*100,'r-',K,Error_tt*100,'blue-')
xlabel('K')
ylabel('Error(%)')
legend('Training error','Testing error','location','southeast')
title('Error rates versus K')
disp('When K ='),disp(K(1)),disp('train error(%)'),disp(Error_tr(1)*100),disp('test error(%)'),disp(Error_tt(1)*100)
disp('When K ='),disp(K(10)),disp('train error(%)'),disp(Error_tr(10)*100),disp('test error(%)'),disp(Error_tt(10)*100)
disp('When K ='),disp(K(end)),disp('train error(%)'),disp(Error_tr(end)*100),disp('test error(%)'),disp(Error_tt(end)*100)
