...
%----------------train-------------------
[train_num ,num_feature_label] = size(data.ytrain);
[test_num ,num_feature_label] = size(data.ytest);
...
[numy1 , num_feature_data] = size(Xtrain1);
[numy0 , num_feature_data] = size(Xtrain0);
mu_theta1 = zeros(2,57) ;                  %store the class conditional(y=1) mean and variance of each feature  
mu_theta0 = zeros(2,57) ;                  %store the class conditional(y=0) mean and variance of each feature  
mu_theta1(1,:) = mean(Xtrain1);     % mean of feature when y = 1
mu_theta1(2,:) = var(Xtrain1)*(numy1-1)/numy1;      % variance of feature when y = 1
mu_theta0(1,:) = mean(Xtrain0);     % mean of feature when y = 0
mu_theta0(2,:) = var(Xtrain0)*(numy0-1)/numy0;      % variance of feature when y = 0
...
for i = 1:train_num
    ...
    for j = 1:num_feature_data
        p1 = p1 + log(normpdf(aXtrain(i,j),mu_theta1(1,j),sqrt(mu_theta1(2,j))));%The probility of y = 1
        p0 = p0 + log(normpdf(aXtrain(i,j),mu_theta0(1,j),sqrt(mu_theta0(2,j))));%The probility of y = 0
    end
...
for i = 1:test_num
    ...
    p0 = 0;%The probility of y = 0
    for j = 1:num_feature_data
        p1 = p1 + log(normpdf(aXtest(i,j),mu_theta1(1,j),sqrt(mu_theta1(2,j))));%The probility of y = 1
        p0 = p0 + log(normpdf(aXtest(i,j),mu_theta0(1,j),sqrt(mu_theta0(2,j))));%The probility of y = 0
    end
    p1 = p1 + log(lambda_ML);%The probility of y = 1
    p0 = p0 + log(1-lambda_ML);%The probility of y = 0
    class_pre(i,1) = p1;
