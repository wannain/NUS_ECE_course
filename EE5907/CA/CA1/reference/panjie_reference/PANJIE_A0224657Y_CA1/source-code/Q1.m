...
for i = 1:57;
    ...
    Px1y1 = sum(feature1);  %when y = 1 , the number of train samples whose Xi = 1
    Px1y0 = sum(feature0);  %when y = 0 , the number of train samples whose Xi = 1
    P_tr(1,i) = Px1y1;                             
    P_tr(2,i) = train_numy1 - Px1y1;                 %when y = 1 , the number of train samples whose Xi = 0
    P_tr(3,i) = Px1y0;
    P_tr(4,i) = train_numy0 - Px1y0;                 %when y = 0 , the number of train samples whose Xi = 0
end
...
title('Error rates versus different α')


disp('When alpha = 1:'),disp('train error(%)'),disp(Error_tr(3)*100),disp('test error(%)'),disp(Error_tt(3)*100)
disp('When alpha = 10:'),disp('train error(%)'),disp(Error_tr(21)*100),disp('test error(%)'),disp(Error_tt(21)*100)
disp('When alpha = 100:'),disp('train error(%)'),disp(Error_tr(201)*100),disp('test error(%)'),disp(Error_tt(201)*100)
